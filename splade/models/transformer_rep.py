...
class SiameseBaseMaxSim(torch.nn.Module, ABC):
    ...
    def __init__(self, model_type_or_dir, output, match="dot_product", model_type_or_dir_q=None, freeze_d_model=False,
        ...
        self.vocab_size = len(self.transformer_rep.tokenizer.vocab)
        ...
        with torch.cuda.amp.autocast() if self.fp16 else NullContextManager():
            ...
            if do_d and do_q:
                if "nb_negatives" in kwargs:
                    ...
                    if "score_batch" in kwargs:
                        score_ = torch.einsum("abc,edc->abde", d_rep_full, q_rep_full) # shape [bs_d, d_pad_len, q_pad_len, bs_q]
                        score_, _ = torch.max(score_, dim=1) # shape [bs_d, q_pad_len, bs_q]
                        score_ = torch.sum(score_, dim=1) # shape [bs_d, bs_q]
                        score = score_
                        
                        score_normal = torch.matmul(q_rep, d_rep.t())  # shape (bs_q, bs_d)
                    else:
                        score_ = torch.einsum("abc,adc->abd", d_rep_full, q_rep_full) # shape [bs, d_pad_len, q_pad_len]
                        score_, _ = torch.max(score_, dim=1) # shape [bs, q_pad_len]
                        score = torch.sum(score_, dim=1, keepdim=True) # shape [bs]
                        # score = torch.sum(q_rep * d_rep, dim=1, keepdim=True)  # shape (bs, )

                        score_normal = torch.sum(q_rep * d_rep, dim=1, keepdim=True)  # shape (bs, )
                out.update({"score": score, "score_normal": score_normal})
        return out
    



class SpladeMaxSim(SiameseBaseMaxSim):
    """SPLADE model
    """
    ...
    def encode(self, tokens, is_q):
        ...
        if self.agg == "sum":
            raise NotImplementedError
            return torch.sum(torch.log(1 + torch.relu(out)) * tokens["attention_mask"].unsqueeze(-1), dim=1), None, None
        else:
            temp = torch.log(1 + torch.relu(out)) * tokens["attention_mask"].unsqueeze(-1)
            values, indices = torch.max(temp, dim=1)

            pad_len = int(temp.size(1))
            return values, indices, pad_len
            # 0 masking also works with max because all activations are positive



class SiameseBasePhrase(torch.nn.Module, ABC):
...
class PhraseSplade(SiameseBasePhrase):
    """PhraseSPLADE model
    """

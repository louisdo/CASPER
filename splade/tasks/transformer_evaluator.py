...
class SparseIndexingMaxSim(SparseIndexing):
    def index(self, collection_loader, id_dict=None):
        doc_ids = []
        if self.compute_stats:
            stats = defaultdict(float)
        ...
        with torch.no_grad():
            for t, batch in enumerate(tqdm(collection_loader)):
                inputs = {k: v.to(self.device) for k, v in batch.items() if k not in {"id"}}
                if self.is_query:
                    batch_documents, batch_documents_token_indices, batch_documents_pad_len = \
                        self.model.encode(inputs, is_q=True)
                else:
                    batch_documents, batch_documents_token_indices, batch_documents_pad_len = \
                        self.model.encode(inputs, is_q=False)
                if self.compute_stats:
                    stats["L0_d"] += self.l0(batch_documents).item()
                row, col = torch.nonzero(batch_documents, as_tuple=True)
                data = batch_documents[row, col]
                data_token_indices = batch_documents_token_indices[row, col]
                data_pad_len = batch_documents_pad_len

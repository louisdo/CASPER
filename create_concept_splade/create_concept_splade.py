...
def create_phrase_vocab(path, num_phrases, threshold = 50):
    # currently using phrase vocab from ERU-KG

    with open(path) as f:
        phrase_metadata = json.load(f)

        phrase_vocab = phrase_metadata["phrase_vocab"]
        docid2phraseid = phrase_metadata["docid2phraseid"]


    phraseidcounter = Counter()
    for phrase_ids in docid2phraseid.values():
        phraseidcounter.update(phrase_ids)

    if threshold is not None: 
        phraseidcounter = Counter({k:v for k,v in phraseidcounter.items() if v >= threshold})

    top_phrase_ids = phraseidcounter.most_common(num_phrases)
    top_phrases = [phrase_vocab[line[0]] for line in top_phrase_ids]

    top_phrases = [phrase for phrase in top_phrases if len(simple_tokenize(phrase)) <=3]
...
def create_model_with_added_phrase_vocab(phrase_vocab, model_name = "distilbert/distilbert-base-uncased", max_added_phrases = 20000):
    ...
    with torch.no_grad():
        ...
        # phrases_to_add_initialization_metadata = create_embedding_and_mlm_for_added_phrases(
        #     phrases_to_add=phrases_to_add, model = model, tokenizer = tokenizer, to_skip_tokens=to_skip_tokens)
        
        # phrases_to_add_embeddings = phrases_to_add_initialization_metadata["embeddings"]
        ...
        # for phrase, phrase_embedding in tqdm(zip(phrases_to_add, phrases_to_add_embeddings)):
        #     index = model_vocab[phrase]

        #     model.distilbert.embeddings.word_embeddings.weight.data[index] = phrase_embedding
...
if __name__ == "__main__":
    path = "/scratch/lamdo/erukg_cache_custom_trained_combined_references_nounphrase_v8-1.json"
    phrase_vocab = create_phrase_vocab(path, 100000, threshold=20)
    ...
    model_name_on_hf = "lamdo/distilbert-base-uncased-phrase-14kaddedphrases"
